...
def train(train_large,path, model_type, epochs, batch_size,learning_rate, suffix=""):
    ...
    # Saving the model
    model.save(f'{savepath}/{model_type}.h5')
    print("------------------------------------------")
    print(f'Model saved')
    print("------------------------------------------")
...
def GAP_feature_TSNE(train_large, path, model_type, batch_size, suffix=""):
    ...
    class_4_indices = np.where(np.array(labels)==3)[0]
    ...
    cls_4 = plt.scatter(tsne_df.iloc[class_4_indices,0], tsne_df.iloc[class_4_indices,1], label='Other',marker='o', s=0.25)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plot_savepath,'GAP_featur_cluster_with_other.png'), dpi=300)
    plt.close()
    ...
    plt.ylabel('2nd component', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(plot_savepath,'GAP_featur_cluster_without_other.png'), dpi=300)
    plt.close()
